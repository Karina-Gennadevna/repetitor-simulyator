...
- В заданиях на выбор слова (орфография, лексика) должен быть РОВНО ОДИН правильный ответ — остальные варианты должны явно не подходить
- НЕ добавляй поле "example" — это спойлер для ученика
${contextGroupNote}
${graph22Note}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!Array.isArray(questions)) return res.status(500).json({ error: 'Claude returned non-array questions' });
